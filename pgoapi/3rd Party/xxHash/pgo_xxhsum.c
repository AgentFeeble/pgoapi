/*
*  pgo_xxhsum - Command line interface for pgo_xxhash algorithms
...
#ifndef pgo_XXHASH_C_2097394837
#define pgo_XXHASH_C_2097394837
...
#ifdef pgo_XXHSUM_INCLUDE_pgo_XXHC    /* compile pgo_xxhsum with pgo_xxhash as private (no public symbol) */
#  define pgo_XXH_INCLUDE_BODY
#else
#  define pgo_XXH_STATIC_LINKING_ONLY   /* *_state_t */
#endif
...
*  Constants
***************************************/
#define LIB_VERSION pgo_XXH_VERSION_MAJOR.pgo_XXH_VERSION_MINOR.pgo_XXH_VERSION_RELEASE
...
#define pgo_XXHSUM32_DEFAULT_SEED 0                   /* Default seed for algo_pgo_xxh32 */
#define pgo_XXHSUM64_DEFAULT_SEED 0                   /* Default seed for algo_pgo_xxh64 */
...
typedef enum { algo_pgo_xxh32, algo_pgo_xxh64 } algoType;
static const algoType g_defaultAlgo = algo_pgo_xxh64;    /* required within main() & usage() */

/* <16 hex char> <SPC> <SPC> <filename> <'\0'>
 * '4096' is typical Linux PATH_MAX configuration. */
#define DEFAULT_LINE_LENGTH (sizeof(pgo_XXH64_hash_t) * 2 + 2 + 4096 + 1)
...
static U32 localpgo_XXH32(const void* buffer, size_t bufferSize, U32 seed) { return pgo_XXH32(buffer, bufferSize, seed); }

static U32 localpgo_XXH64(const void* buffer, size_t bufferSize, U32 seed) { return (U32)pgo_XXH64(buffer, bufferSize, seed); }
...
{
    /* pgo_XXH32 bench */
    BMK_benchHash(localpgo_XXH32, "pgo_XXH32", buffer, bufferSize);

    /* Bench pgo_XXH32 on Unaligned input */
    if (bufferSize>1)
        BMK_benchHash(localpgo_XXH32, "pgo_XXH32 unaligned", ((const char*)buffer)+1, bufferSize-1);

    /* Bench pgo_XXH64 */
    BMK_benchHash(localpgo_XXH64, "pgo_XXH64", buffer, bufferSize);

    /* Bench pgo_XXH64 on Unaligned input */
    if (bufferSize>1)
        BMK_benchHash(localpgo_XXH64, "pgo_XXH64 unaligned", ((const char*)buffer)+1, bufferSize-1);
}
...
{
    pgo_XXH64_state_t state;
    U64 Dresult;
    int index;

    Dresult = pgo_XXH64(sentence, len, seed);
    BMK_checkResult64(Dresult, Nresult);

    pgo_XXH64_reset(&state, seed);
    pgo_XXH64_update(&state, sentence, len);
    Dresult = pgo_XXH64_digest(&state);
    BMK_checkResult64(Dresult, Nresult);

    pgo_XXH64_reset(&state, seed);
    for (index=0; index<len; index++) pgo_XXH64_update(&state, ((char*)sentence)+index, 1);
    Dresult = pgo_XXH64_digest(&state);
    BMK_checkResult64(Dresult, Nresult);
}
...
{
    pgo_XXH32_state_t state;
    U32 Dresult;
    size_t index;

    Dresult = pgo_XXH32(sequence, len, seed);
    BMK_checkResult(Dresult, Nresult);

    pgo_XXH32_reset(&state, seed);
    pgo_XXH32_update(&state, sequence, len);
    Dresult = pgo_XXH32_digest(&state);
    BMK_checkResult(Dresult, Nresult);

    pgo_XXH32_reset(&state, seed);
    for (index=0; index<len; index++) pgo_XXH32_update(&state, ((const char*)sequence)+index, 1);
    Dresult = pgo_XXH32_digest(&state);
    BMK_checkResult(Dresult, Nresult);
}
...
static void BMK_hashStream(void* pgo_xxhHashValue, const algoType hashType, FILE* inFile, void* buffer, size_t blockSize)
{
    pgo_XXH64_state_t state64;
    pgo_XXH32_state_t state32;
    size_t readSize;

    /* Init */
    pgo_XXH32_reset(&state32, pgo_XXHSUM32_DEFAULT_SEED);
    pgo_XXH64_reset(&state64, pgo_XXHSUM64_DEFAULT_SEED);
    ...
    while (readSize) {
        ...
        {
        case algo_pgo_xxh32:
            pgo_XXH32_update(&state32, buffer, readSize);
            break;
        case algo_pgo_xxh64:
            pgo_XXH64_update(&state64, buffer, readSize);
            break;
        default:
            break;
        }
    ...
    {
    case algo_pgo_xxh32:
        {   U32 const h32 = pgo_XXH32_digest(&state32);
            memcpy(pgo_xxhHashValue, &h32, sizeof(h32));
            break;
        }
    case algo_pgo_xxh64:
        {   U64 const h64 = pgo_XXH64_digest(&state64);
            memcpy(pgo_xxhHashValue, &h64, sizeof(h64));
            break;
        }
    default:
            break;
                    ...
                    const endianess displayEndianess)
{
    ...
    {
    case algo_pgo_xxh32:
        BMK_hashStream(&h32, algo_pgo_xxh32, inFile, buffer, blockSize);
        break;
    case algo_pgo_xxh64:
        BMK_hashStream(&h64, algo_pgo_xxh64, inFile, buffer, blockSize);
        break;
    default:
        break;
    }
    ...
    {
    case algo_pgo_xxh32:
        {   pgo_XXH32_canonical_t hcbe32;
            pgo_XXH32_canonicalFromHash(&hcbe32, h32);
    ...
    case algo_pgo_xxh64:
        {   pgo_XXH64_canonical_t hcbe64;
            pgo_XXH64_canonicalFromHash(&hcbe64, h64);
            displayEndianess==big_endian ?
...
typedef union {
    pgo_XXH32_canonical_t pgo_xxh32;
    pgo_XXH64_canonical_t pgo_xxh64;
} Canonical;

typedef struct {
    Canonical   canonical;
    const char* filename;
    int         pgo_xxhBits;    /* canonical type : 32:pgo_xxh32, 64:pgo_xxh64 */
} ParsedLine;
...
/*  Converts pgo_XXH32 canonical hexadecimal string hashStr to big endian unsigned char array dst.
...
/*  Parse single line of pgo_xxHash checksum file.
 ...
 *
 *  Given pgo_xxHash checksum line should have the following format:
 *
...
static ParseLineResult parseLine(ParsedLine* parsedLine, const char* line)
{
    ...
    {
    case 8:
        {   pgo_XXH32_canonical_t* pgo_xxh32c = &parsedLine->canonical.pgo_xxh32;
            if (canonicalFromString(pgo_xxh32c->digest, sizeof(pgo_xxh32c->digest), line)
                != CanonicalFromString_ok) {
                return ParseLine_invalidFormat;
            }
            parsedLine->pgo_xxhBits = 32;
            break;
        }

    case 16:
        {   pgo_XXH64_canonical_t* pgo_xxh64c = &parsedLine->canonical.pgo_xxh64;
            if (canonicalFromString(pgo_xxh64c->digest, sizeof(pgo_xxh64c->digest), line)
                != CanonicalFromString_ok) {
                return ParseLine_invalidFormat;
            }
            parsedLine->pgo_xxhBits = 64;
            break;
        }
...
/*!  Parse pgo_xxHash checksum file.
 */
    ...
    while (!report->quit) {
        ...
        if (parseLine(&parsedLine, parseFileArg->lineBuf) != ParseLine_ok) {
            ...
            if (parseFileArg->warn) {
                DISPLAY("%s : %lu: improperly formatted pgo_XXHASH checksum line\n"
                    , inFileName, lineNumber);
            }
            continue;
        }

        if (report->pgo_xxhBits != 0 && report->pgo_xxhBits != parsedLine.pgo_xxhBits) {
            /* Don't accept pgo_xxh32/pgo_xxh64 mixed file */
            report->nImproperlyFormattedLines++;
            report->nMixedFormatLines++;
            if (parseFileArg->warn) {
                DISPLAY("%s : %lu: improperly formatted pgo_XXHASH checksum line (pgo_XXH32/64)\n"
                    , inFileName, lineNumber);
            }
            continue;
        }
        ...
        if (report->pgo_xxhBits == 0) {
            report->pgo_xxhBits = parsedLine.pgo_xxhBits;
        }
        ...
        if (fp == NULL) {
            ...
            switch (parsedLine.pgo_xxhBits)
            {
            case 32:
                {   pgo_XXH32_hash_t pgo_xxh;
                    BMK_hashStream(&pgo_xxh, algo_pgo_xxh32, fp, parseFileArg->blockBuf, parseFileArg->blockSize);
                    if (pgo_xxh == pgo_XXH32_hashFromCanonical(&parsedLine.canonical.pgo_xxh32)) {
                        lineStatus = LineStatus_hashOk;
                }   }
                break;

            case 64:
                {   pgo_XXH64_hash_t pgo_xxh;
                    BMK_hashStream(&pgo_xxh, algo_pgo_xxh64, fp, parseFileArg->blockBuf, parseFileArg->blockSize);
                    if (pgo_xxh == pgo_XXH64_hashFromCanonical(&parsedLine.canonical.pgo_xxh64)) {
                        lineStatus = LineStatus_hashOk;
                }   }
                break;
                     ...
                     U32 quiet)
{
    ...
    if (inFileName == stdinName) {
        /* note : Since we expect text input for pgo_xxhash -c mode,
    ...
    if (report->nProperlyFormattedLines == 0) {
        DISPLAY("%s: no properly formatted pgo_XXHASH checksum lines found\n", inFileName);
    } else if (!statusOnly) {
...
static int usage(const char* exename)
{
    ...
    DISPLAY( " -c  : read pgo_xxHash sums from the [filenames] and check them\n");
    DISPLAY( " -h  : help \n");
    return 0;
}
...
int main(int argc, const char** argv)
{
    ...
    /* special case : pgo_xxh32sum default to 32 bits checksum */
    if (strstr(exename, "pgo_xxh32sum") != NULL) algo = algo_pgo_xxh32;
